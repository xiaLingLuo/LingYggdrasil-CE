...
插件 API 版本与主程序版本**同步发布**，当前为 `2.2.2`（取自 `pom.xml`）。

- 单值 `2.2.2` → 必须**精确等于**服务端 API 版本才兼容。
- 区间 `2.2.0to2.2.2` → 服务端版本落在闭区间 `[2.2.0, 2.2.2]` 内即兼容。
...
```text
target/LingYggdrasil-plugin-api-2.2.2.jar
```
...
```bash
javac -cp LingYggdrasil-plugin-api-2.2.2.jar -d out src/**/*.java
