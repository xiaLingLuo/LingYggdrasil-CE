...
(function() {
    async function loadSkins() {
        var container = document.getElementById('skinList');
        if (!container) return;
        ...
        try {
            ...
            if (data.success && data.skins && data.skins.length > 0) {
                ...
                data.skins.forEach(function(s) {
                    var displayName = s.alias || s.originalName || s.hash;
                    var previewUrl = '/api/skins/download?id=' + encodeURIComponent(s.id);
                    var visColor = s.isPublic ? '#0bda51' : '#5897fb';
                    var visLabel = s.isPublic ? t('texture.publicLabel') : t('texture.privateLabel');
                    html += '<div class="texture-item card-animate texture-card-clickable" id="skin-' + escapeHtml(s.id) + '"' +
                        ' data-id="' + escAttr(s.id) + '"' +
                        ' data-alias="' + escAttr(s.alias || '') + '"' +
                        ' data-original-name="' + escAttr(s.originalName || '') + '"' +
                        ' data-hash="' + escAttr(s.hash) + '"' +
                        ' data-size="' + escAttr(String(s.size)) + '"' +
                        ' data-vis="' + (s.isPublic ? '1' : '0') + '">' +
                        '<div class="texture-thumb"><canvas></canvas></div>' +
                        '<div class="texture-item-body">' +
                        '<div class="texture-name">\u273F ' + escapeHtml(displayName) + '</div>' +
                        '<div class="texture-meta">' + formatSize(s.size) + '</div>' +
                        '</div>' +
                ...
                html += '<div class="upload-tile card-animate" id="skinUploadTile">' +
                    '<div class="upload-tile-icon">' +
                    '<svg viewBox="0 0 24 24"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>' +
                    '</div>' +
                    '<div class="upload-tile-text">' + t('texture.uploadSkin') + '</div>' +
                ...
                container.querySelectorAll('.texture-thumb canvas').forEach(function(c) {
                    ...
                    var url = '/api/skins/download?id=' + encodeURIComponent(card.dataset.id);
                    drawFace(c, url);
                });
            } else {
                container.innerHTML = '<p class="empty-hint" style="grid-column:1/-1">' + t('texture.emptySkins') + ' <i class="fas fa-leaf"></i></p>' +
    ...
    var gridContainer = document.getElementById('skinList');

    if (gridContainer) {
        gridContainer.addEventListener('click', function(e) {
            if (e.target.closest('#skinUploadTile')) fileInput.click();
        });
        ['dragenter', 'dragover'].forEach(function(evt) {
            gridContainer.addEventListener(evt, function(e) {
                e.preventDefault(); e.stopPropagation();
                var tile = document.getElementById('skinUploadTile');
    ...
    window.showSkinDetail = function(id, alias, originalName, hash, size) {
        var existing = document.getElementById('detailModal');
        if (existing) existing.remove();

        var displayName = alias || originalName || hash;
        var previewUrl = '/api/skins/download?id=' + encodeURIComponent(id);
        ...
        modalBox.innerHTML =
            ...
            '<div class="detail-preview"><canvas id="skin3dCanvas"></canvas></div>' +
            '<div class="detail-actions" id="detailActions"></div>';

        overlay.appendChild(modalBox);
        document.body.appendChild(overlay);

        window._skinViewer = null;
        if (typeof skinview3d !== 'undefined') {
            try {
                var canvas = document.getElementById('skin3dCanvas');
                var previewDiv = canvas.parentElement;
                window._skinViewer = new skinview3d.SkinViewer({
                    canvas: canvas,
                ...
                window._skinViewer.autoRotate = true;
                window._skinViewer.animation = new skinview3d.WalkingAnimation();
            } catch(e) {  }
        }
        ...
        dlBtn.addEventListener('click', function() { downloadSkin(id); });
        actionsDiv.appendChild(dlBtn);

        var applyBtn = document.createElement('button');
        applyBtn.className = 'btn btn-primary';
        applyBtn.textContent = t('texture.applyToProfile');
        applyBtn.addEventListener('click', function() { applyTextureToProfile('SKIN', hash); });
        ...
        delBtn.addEventListener('click', function() { deleteSkin(id, displayName); });
    ...
    window.saveAlias = async function(id) {
        ...
        try {
            var resp = await fetch('/api/skins/alias', {
                method: 'POST',
            ...
            if (data.success) {
                setTimeout(function() { closeAliasModal(); closeDetailModal(); loadSkins(); }, 800);
            }
        } catch (err) {
    ...
    window.downloadSkin = function(id) {
        window.open('/api/skins/download?id=' + encodeURIComponent(id), '_blank');
    };

    window.deleteSkin = function(id, name) {
        showConfirmDialog(t('texture.deleteConfirm', t('texture.skin'), name), function() {
            fetch('/api/skins/delete', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': (window.CSRF_TOKEN || '') },
    ...
    async function submitUpload(file) {
        ...
        try {
            var resp = await fetch('/api/skins/upload', {
                method: 'POST',
