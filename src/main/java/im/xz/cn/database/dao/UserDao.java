...
public class UserDao {
    ...
    public void insert(User user) {
        db.executeUpdate(
            "INSERT INTO users (id, username, email, password_hash, nickname, email_verified, created_at, last_login, registered_ip, last_login_ip, friend_code, perm_group) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            user.getId(), user.getUsername(), user.getEmail(), user.getPasswordHash(),
            user.getNickname(), user.isEmailVerified() ? 1 : 0,
            user.getCreatedAt(), user.getLastLogin(), user.getRegisteredIp(), user.getLastLoginIp(),
            user.getId() != null ? im.xz.cn.common.UuidUtil.generateFriendCode(user.getId()) : null,
            user.getPermGroup() == null ? "default" : user.getPermGroup()
        );
    }
    ...
    public List<User> findAll() {
        ...
        try (Connection conn = db.getConnection();
             PreparedStatement ps = conn.prepareStatement("SELECT * FROM users WHERE id != '-99' ORDER BY created_at ASC");
    ...
    public void update(User user) {
        db.executeUpdate(
            "UPDATE users SET username = ?, email = ?, password_hash = ?, nickname = ?, email_verified = ?, last_login = ?, registered_ip = ?, last_login_ip = ? WHERE id = ?",
    ...
    public int count() {
        var result = db.executeQuerySingle("SELECT COUNT(*) AS cnt FROM users WHERE id != '-99'");
    ...
    public void updateTheme(String userId, String theme) {
        db.executeUpdate("UPDATE users SET theme = ? WHERE id = ?", theme, userId);
    }

    public void updateLanguage(String userId, String language) {
        db.executeUpdate("UPDATE users SET language = ? WHERE id = ?", language, userId);
    }

    public void updatePermGroup(String userId, String permGroup) {
        db.executeUpdate("UPDATE users SET perm_group = ? WHERE id = ?", permGroup, userId);
    }
