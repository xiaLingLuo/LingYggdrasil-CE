...
public class PermGroupDao {
    private static final logApi log = logApi.getLogger(PermGroupDao.class);
    private final DatabaseManager db;

    public PermGroupDao(DatabaseManager db) {
        this.db = db;
    }
    ...
    public PermGroup findById(String id) {
        return querySingle("SELECT * FROM op_perm_group WHERE id = ?", id);
    }

    public PermGroup findByName(String name) {
        return querySingle("SELECT * FROM op_perm_group WHERE name = ?", name);
    }

    public List<PermGroup> findAll() {
        List<PermGroup> list = new ArrayList<>();
        try (Connection conn = db.getConnection();
             PreparedStatement ps = conn.prepareStatement("SELECT * FROM op_perm_group ORDER BY created_at ASC");
    ...
    public void updatePermissions(String id, String permissions) {
        db.executeUpdate("UPDATE op_perm_group SET permissions = ? WHERE id = ?", permissions, id);
    }

    public void updateName(String id, String name) {
        db.executeUpdate("UPDATE op_perm_group SET name = ? WHERE id = ?", name, id);
    }

    public void delete(String id) {
        db.executeUpdate("DELETE FROM op_perm_group WHERE id = ?", id);
    }

    public int count() {
        var row = db.executeQuerySingle("SELECT COUNT(*) AS cnt FROM op_perm_group");
    ...
    public int countAdminsUsing(String groupName) {
        var row = db.executeQuerySingle("SELECT COUNT(*) AS cnt FROM admins WHERE perm_group = ?", groupName);
        if (row != null && row.get("cnt") != null) {
            return ((Number) row.get("cnt")).intValue();
        }
        return 0;
