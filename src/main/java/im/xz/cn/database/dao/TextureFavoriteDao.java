...
public class TextureFavoriteDao {
    private static final logApi log = logApi.getLogger(TextureFavoriteDao.class);
    ...
    public Map<String, Integer> batchCountByTextures(List<String> textureIds) {
        ...
        for (int offset = 0; offset < textureIds.size(); offset += batchSize) {
            ...
            var rows = db.executeQuery("SELECT texture_id, COUNT(*) AS cnt FROM texture_favorites WHERE texture_id IN (" + placeholders + ") GROUP BY texture_id", batch.toArray());
            for (var row : rows) {
