...
public class PngNormalizer {
    ...
    private static final logApi LOGGER = logApi.getLogger(PngNormalizer.class);
    ...
    public static class Builder {
        ...
        private long maxPixels = 100_000_000;   
    ...
    private byte[] generatePurePng(BufferedImage sourceImage) throws IOException {
        ...
        for (int y = 0; y < height; y++) {
            ...
            for (int x = 0; x < width; x++) {
                ...
                rawStream.write((argb >> 16) & 0xFF); 
                rawStream.write((argb >> 8) & 0xFF);  
                rawStream.write(argb & 0xFF);         
                if (hasTransparency) {
                    rawStream.write((argb >> 24) & 0xFF); 
                }
            }
        }
