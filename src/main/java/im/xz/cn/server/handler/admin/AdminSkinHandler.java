...
public class AdminSkinHandler {
    ...
    public AdminSkinHandler(TextureDao textureDao, TextureService textureService, UserDao userDao, DatabaseManager db, SystemConfig systemConfig) {
    ...
    public void getSkins(Context ctx) {
        if (!im.xz.cn.security.AdminPermissions.require(ctx, "admin.skins.view")) return;
        if (!isRoot(ctx)) {
    ...
    public void uploadSkin(Context ctx) {
        ...
        ctx.json(Map.of("success", false, "message", I18n.t("msg.adminNoUploadSkin")));
    }

    @SuppressWarnings("unchecked")
    public void deleteSkin(Context ctx) {
        if (!im.xz.cn.security.AdminPermissions.require(ctx, "admin.skins.delete")) return;
        if (!isRoot(ctx)) {
            ctx.json(Map.of("success", false, "message", I18n.t("msg.rootOnly")));
            return;
        }
    ...
    public void deleteOrphanSkins(Context ctx) {
        if (!im.xz.cn.security.AdminPermissions.require(ctx, "admin.skins.orphans")) return;
        if (!isRoot(ctx)) {
            ctx.json(Map.of("success", false, "message", I18n.t("msg.rootOnly")));
            return;
        }
        int deleted = deleteOrphans("SKIN");
        AuditLogger.logSensitiveOperation(getAdminName(ctx), "DELETE_ORPHAN_SKINS:" + deleted, IpUtil.getClientIp(ctx));
        ctx.json(Map.of("success", true, "message", I18n.t("msg.deletedOrphanSkins", deleted)));
    }
    ...
    public void updateAlias(Context ctx) {
        if (!im.xz.cn.security.AdminPermissions.require(ctx, "admin.skins.alias")) return;
        if (!isRoot(ctx)) {
            ctx.json(Map.of("success", false, "message", I18n.t("msg.rootOnly")));
            return;
        }
    ...
    public void downloadSkin(Context ctx) {
        if (!im.xz.cn.security.AdminPermissions.require(ctx, "admin.skins.download")) return;
