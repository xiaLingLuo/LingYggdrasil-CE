...
import im.xz.cn.server.handler.yggdrasil.YggdrasilAuthHandler;
import im.xz.cn.server.handler.yggdrasil.YggdrasilSessionHandler;
...
public class YggdrasilServer {
    private static final logApi logger = logApi.getLogger(YggdrasilServer.class);
    ...
    public YggdrasilServer(DatabaseManager db) {
        ...
        this.app = ServerFactory.create(35577, null, routes -> {
            ...
            routes.post("/api/profiles/lookup/minecraft", ctx -> {
                ...
                int maxBatch = im.xz.cn.config.SystemConfig.getInstance().getBatchQueryMaxCount();
                if (maxBatch > 0 && names.size() > maxBatch) {
                    logger.warn("[Yggdrasil] 批量查询角色 超出上限: 请求{}个, 上限{}", names.size(), maxBatch);
                    ctx.status(400).contentType("application/json");
                    ctx.json(Map.of("error", "IllegalArgumentException",
                            "errorMessage", "Too many profiles requested (max " + maxBatch + ")"));
                    return;
                }
    ...
    private void handleMetadata(Context ctx) {
        ...
        Map<String, Object> meta = getMeta(sysConfig);
    ...
    @NotNull
    private static Map<String, Object> getMeta(SystemConfig sysConfig) {
